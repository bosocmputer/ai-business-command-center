...
MAX_GRAPH_BYTES="${GRAPHIFY_MAX_GRAPH_BYTES:-15000000}"
...
if find graphify-out -type f \( \
  ...
  -name "*.pdf" -o -name "*.mp4" -o -name "*.mov" -o -name "*.mp3" -o \
  -name "*.wav" \
  \) | grep -q .; then
  find graphify-out -type f \( \
    -name ".env" -o -name ".env.*" -o -name "*.pem" -o -name "*.key" -o \
    -name "*.p12" -o -name "*.pfx" -o -name "*.sqlite" -o -name "*.sqlite3" -o \
    -name "*.db" -o -name "*.dump" -o -name "*.bak" -o -name "*.png" -o \
    -name "*.jpg" -o -name "*.jpeg" -o -name "*.webp" -o -name "*.gif" -o \
...
if grep -Eiq '(^|[/"])(\.env|artifacts|\.data|node_modules|dist|\.next|coverage|screenshots?|pdf-cache)([/"]|$)' "$GRAPH_PATH"; then
...
STALE_FILE="$(
  find apps/api/src apps/web/src/components/owner apps/web/src/components/command-center \
    apps/worker/src packages/shared/src packages/reports/src infra scripts docs/adr docs/reports \
    -type f \( \
